...
Package-authored wrapper surfaces and public `unknown` boundaries also need at least one direct repo-owned test that exercises the public call shape itself when the public claim is that ordinary users can use the surface directly.
...
- a package-internal test that satisfies a public `unknown` boundary through package-local `%identity` support casts
...
- does at least one proof exercise the public call shape directly instead of package-local support casts
