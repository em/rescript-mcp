...
- Adding new package-authored wrapper surface
...
This package is in changesets pre-release mode (`npx changeset pre enter alpha`). The `.changeset/pre.json` file controls this. While active, all versions produced by `changeset version` are `X.Y.Z-alpha.N` and published to the `alpha` dist-tag. `npm install rescript-mcp` will not resolve to alpha versions — consumers must explicitly `npm install rescript-mcp@alpha`.
...
- The workflow runs build and tests before the publish step. If either fails, nothing publishes.
